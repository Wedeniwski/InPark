<?php
  ...
  class Plist {
    ...
    public static function from_file($file) {
      if (!file_exists($file)) throw new Exception("Unable to open plist file.");
    ...
    public function __toString() {
      if ($xml = $this->as_xml()) return $xml;
      if (!empty($this->data['Label'])) return $this->data['Label'];
      return NULL;
    }
    
    public function __get($key) {
      if (!empty($this->data[$key])) return $this->data[$key];
      return NULL;
    }
    
    public function as_xml() {
      if (is_a($this->document, "DOMDocument")) return $this->document->saveXML();
      return NULL;
    }
    ...
    public static function parse_value($value_node) {
      ...
      $class = get_class();
      if (is_callable("{$class}::{$transformer_name}")) return call_user_func(array($class, $transformer_name), $value_node);
      return NULL;
    }
    ...
    public static function parse_dict($dict_node) {
      ...
      for ($node = $dict_node->firstChild; $node != null; $node = $node->nextSibling) {
        if ($node->nodeName == "key") {
          ...
          while ($value_node->nodeType == XML_TEXT_NODE ) $value_node = $value_node->nextSibling; // skip text nodes
    ...
    public static function parse_array( $array_node ) {
      ...
      for ($node = $array_node->firstChild; $node != null; $node = $node->nextSibling) {
        if ($node->nodeType == XML_ELEMENT_NODE) array_push($array, self::parse_value($node));
      }
      return $array;
    }
  } // End of class Plist
  ...
  $plist = Plist::from_file($filename);
  print_r($plist->as_array());
?>
