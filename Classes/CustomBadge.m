...
-(void)drawRoundedRectWithContext:(CGContextRef)context withRect:(CGRect)rect {
  ...
  CGFloat maxY = CGRectGetMaxY(rect);
	const CGFloat radius = maxY*self.badgeCornerRoundness;
	const CGFloat puffer = maxY*0.10;
	const CGFloat maxX = CGRectGetMaxX(rect) - puffer;
	const CGFloat minX = CGRectGetMinX(rect) + puffer;
	const CGFloat minY = CGRectGetMinY(rect) + puffer;
	maxY -= puffer;
...
-(void) drawShineWithContext:(CGContextRef)context withRect:(CGRect)rect {
  ...
  CGFloat maxY = CGRectGetMaxY(rect);
	...
	const CGFloat locations[2] = { 0.0, 0.4 };
	const CGFloat components[8] = {  0.92, 0.92, 0.92, 1.0, 0.82, 0.82, 0.82, 0.4 };
...
-(void)drawRect:(CGRect)rect {
	...
	if (self.badgeShining) [self drawShineWithContext:context withRect:rect];
  const int n = (int)[self.badgeText length];
	if (n > 0) {
		[badgeTextColor set];
		CGFloat sizeOfFont = 13.5*badgeScaleFactor;
		if (n < 2) sizeOfFont += sizeOfFont*0.20;
		UIFont *textFont = [UIFont boldSystemFontOfSize:sizeOfFont];
    NSRange range = [badgeText rangeOfString:@"\n"];
    if (range.length <= 0) {
      ...
      [self.badgeText drawInRect:CGRectInset(rect, 10.0f, 10.0f) withFont:textFont lineBreakMode:UILineBreakModeWordWrap];
    }
	}
}
